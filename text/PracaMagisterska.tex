...
\label{sec:experiments}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\draw  [rounded corners, text centered, fill=blue!20] (0,0) rectangle (4,1.2);
			...
			\node [arch_part_data] (model_lang) at (\l, -3) {\labelBlock{lang_model_clarin}Model akustyczny oraz językowy ze slownikiem};
			\path [line] (htk) -- (model_lang);
			
			\node [arch_part_data] (model_acc) at (\c, 5) {\labelBlock{model_acc4}Model akustyczny};
			...
			\node [arch_part_results] (results) at (\x, 2.5) {\labelBlock{results1}Skuteczność modelu klasycznego};
			...
			\draw  [rounded corners, text centered, fill=blue!20] (0,-6) rectangle (4,-4.8);
			...
			\node [arch_part_data] (siec) at (\c, -7.5) {\labelBlock{net}Sieć neuronowa};
			...
			\node [arch_part_results] (model_acc2) at (\x, -3) {\labelBlock{results2}Skuteczność modelu neuronowego};
			\path [line] (julius3) -- (model_acc2);
			
			\node [arch_part_data] (model_lang2) at (\x, 0.6) {\labelBlock{lang_model2}Zewnętrzny model językowy};
		...
		\end{tikzpicture}
		\caption{Zależności pomiędzy wykorzystanymi komponentami. Na fioletowo zaznaczono dane oraz modele, białe są programy i skrypty, kolorem zielonym oznaczono pomiary skuteczności.}
		\label{fig:arch}	
	\end{figure}
