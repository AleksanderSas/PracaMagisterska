	...
	\begin{figure}
		\begin{algorithmic}[1]
			...
			\STATE $X = \emptyset$
				...
				\STATE $P = \Big\{(q_1, q_2, q_3) \in Phones: q_1 = q, q_2 \in Q, q_3 \in Q\Big\}$
				\FOR {$r \in Rules \label{line:loop_R}$}
					...
					\IF {$|f_l| > n \AND |f_r| > n \label{line:if_n}$}
						...
						\STATE $X = X \cup (prob, r, q)$ \label{line:r_prob}
					\ENDIF
				\ENDFOR
			\ENDFOR
			
			\RETURN $\argmax_{(prob, r, q)\in X}(prob)$;
		\end{algorithmic}
