...
\polishtitle{Sieci neuronowe rozpoznające trifony w procesie analizy mowy}
\englishtitle{Neural network recognizing triphones in speech analyze}
...
\chapter{Proces automatycznego rozpoznawania mowy (ARM) }
	...
	\textbf{Automatyczne rozpoznawanie mowy} możemy formalnie zdefiniować, jako znajdowanie ciągu słów $\hat{W}$ z pewnego zbiory $V$, o maksymalnym prawdopodobieństwie, pod warunkiem obserwacji $O$. Zbiór $V$ odpowiada blokowi \refBlock{slownik} z rysunku \ref{fig:ARM_schemat}.
	...
	Niestety wyliczanie formuły \ref{equation:ASR_definicja1} okazuję się niewygodne, ale korzystając ze wzory Bayesa możemy dojść do postaci \ref{equation:ASR_definicja2}, która jest już wygoda do obliczenia.
	...
	\section{ Modelowanie fonemów, modele kontekstowe i bezkontekstowe }
		...
		\\
		Rozwinięciem fonemów są \textbf{trifony}, uwzględniają one lewy i prawy kontekst poprzez dodanie informacji o fonemach stojących obok. Typowo trifony zapisuje się zgodnie z notacją na rysunku \ref{fig:tri-fony_notacja}, gdzie \textit{a} jest poprzednim fonemem, \textit{b} aktualnym, natomiast \textit{c} następnym. W praktyce nie występują wszystkie kombinacje trójek fonemów, takich jak przykładowo \textit{o-o+a}, niemniej jednak wiele z nich można spotkać. W modelach wykorzystanych podczas eksperymentów występuje $27005$ uni, bi oraz trifonów.
...
\section {Rozpoznawanie mowy z zastosowaniem ukrytych modeli Markowa}
    ...
    \subsection{Ukryte modele Markowa - definicja}
	   ...
	   \subsection{Algorytmy Viterbiego}
		   ...
		   \begin{equation}
		   Q_t^q=\sum_{q \in Q} \bigg( P_{t-1}^q\cdot a_{q,w}\cdot b_q(o) \bigg)
...
\section{N-gramowy model językowy}
	\label{sec:ngramm}
	\textit{N-gramowy model językowy}, odpowiadający blokowi \refBlock{n_gramy} z rysunku \ref{fig:ARM_schemat}, zawiera prawdopodobieństwa słów ze słownika \refBlock{slownik}. Model o stopniu $n$ opisuje szansę wystąpienia słowa $w$ poprzedzonego $n-1$ słowami, co można zapisać jako: $P(w|w_{n-1},w_{n-2},\cdots,w_1)$. Oczywiście N-gramowy model zawiera w sobie również modele o niższy stopniu, które są wykorzystywane na początku zdania. Modele językowe buduje się przez zliczanie wystąpień kombinacji słów w pewnym korpusie treningowym. Wraz ze wzrostem $n$ znacząco rośnie ilość tekstu potrzebnego do precyzyjnej estymacji prawdopodobieństw oraz liczba kombinacji słów $w, w_1, \cdots, w_{n-1}$ jakie mogą wystąpić w języku naturalnym. W konsekwencji, przy ograniczonej liczbie wypowiedzi, niektóre kombinacje z warunkowego członu prawdopodobieństwa  $P(w|w_{n-1},w_{n-2},\cdots,w_1)$ nie występują w korpusie treningowym. Aby umożliwić rozpoznanie takiego słowa $w$, poprzedzonego słowami $w_{n-1},w_{n-2},\cdots,w_1$ stosuje się techniki \textit{wygładzania} (ang: \textit{smoothing}) takie jak przykładowo \textit{Add-one} czy \textit{Katz smoothing}. Ideą tych algorytmów jest przypisanie niewystępującym kombinacjom niezerowego prawdopodobieństwa. Wykorzystane w eksperymentach modele językowe miały stopień $3$.
...
\section{Rozpoznawanie mowy z wykorzystaniem sieci neuronowych}
	...
	\subsection{Splotowe sieci neuronowe}
		...
		Warstwa splotowa składa z zadanej parametrem $n$ liczby filtrów, o ustalonym kształcie $a\times b$. Podczas treningu wyznaczane są współczynniki filtrów. Wejściem warstwy jest macierzy trójwymiarowa o wymiarach $x \times y \times k$, natomiast wyjściem jest macierz trójwymiarowa o wymiarach $x - a + 1 \times y - b + 1 \times n$. Podobnie jak przy klasycznych warstwach, tutaj również na wyjście nakłada się funkcje aktywacji $f$. Pojedyncze wyjście $Output_{p,q,m}$ opisane jest wzorem \ref{eqn:nn_conv}, gdzie $Input[p:p+a, q:q+b, i]$ jest podmacierzą natomiast $W_m$ współczynnikami $m$-tego filtra. Operacja splotu $\ast$ dla macierzy $W$, $M$ o wymiarach $a\times b\times c$ zdefiniowana jest wzorem \ref{eqn:conv}.
		...
		\begin{equation}
			W \ast M = \sum_{i=1}^a \sum_{j=1}^b \sum_{k=1}^c \bigg( W[a,b,c] \cdot M[a,b,c] \bigg)
			\label{eqn:conv}
		\end{equation}
	...
	\label{sec:nn_estymator}
		...
		Zgodnie z równaniem \ref{equation:ASR_definicja2} do rozpoznawania mowy potrzeba $P(O|W)$, co oznacza, że klasyfikator musi wyznaczać $P(v|w_i)$. Korzystając ze wzoru Bayes i obserwacji, że $P(v)$ nie ma wpływu na rozpoznawanie, gdyż jest takie samo dla wszystkich hipotez, otrzymujemy:
...
\section{ Zastosowanie modelowania fonemów z ograniczonym kontekstem w ASR }	
	...
	\begin{figure}
		...
		\end{tikzpicture}
		\caption{Tree-based clustering - nakładanie pytań}
...
\label{sec:experiments}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\node [arch_part] (model_lang) at (\l, -3) {\labelBlock{lang_model_clarin}Model akustyczny oraz językowy ze slownikiem};
	...
	\section{Budowa modeli klasycznych}
		...
		\begin{figure}
			...
			\caption{Procedura uczenia trifonowych modeli akustycznych.}
